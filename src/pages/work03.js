...
import Project03 from "../components/relatedProject03"
...
class WorkPage03 extends Component {
  constructor(props) {
    super(props);
    this.state = {
      nav1: null,
  ...
  render() {
    ...
    return (
      <Work>
        <SEO title="居酒屋QUATTOR梅田本店" />
        <main>
          <StaticQuery 
            query={graphql`
              query Work03 {
                allFile(filter: {relativeDirectory: {eq: "work/work03"}}, sort: {fields: base}) {
                  edges {
                    node {
                      base
                      id
          ...
          <section>
            <div className={workStyles.innerWrapper}>
              <h2>BRANDING</h2>
              <p className={workStyles.workTitle}>居酒屋QUATTOR梅田本店</p>
              <p className={workStyles.workContent}>看板・メニュー・外観置き看板・ポスター・ビラ制作<br />オープンから関わることによりデザインの統一化をし、その後もお客様のデータを元にメニュー改善。メニューはiPadを使用。<br />制作期間約1ヶ月<br /><br />Signs, menus, exterior signs, posters, leaflets<br />We have unified the design by being involved since the opening, and have continued to improve the menu based on customer data.<br />Production period about 1 month</p>
            </div>
          </section>
          <Project03 />
        </main>
